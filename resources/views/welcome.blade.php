...
@include('components.request-service')
	...
	<section class="hero-section">
		...
		@endif
			<div class="row">
				<div class="col-lg-6">
					<div class="hs-text">
						<h2 style="color:#f1f1f1">{{__('My-EMARKET')}}</h2>
						<p>{{ __('The most convenient property estate search portal in Zambia, start searching now!') }}</p>
						<form action="{{route('property.index')}}" method="GET">
						<input type="hidden" name="type" value="4" readonly>
						<button type="submit" class="site-btn sb-dark">{{__('Filter Items')}} <i class="fas fa-smile"></i></button>
					    </form>
						<br>	
						<!-- Trigger Button -->
						<button type="button" class="site-btn" data-bs-toggle="modal" data-bs-target="#requestServiceModal">
							{{ __('Request For a Service') }}
						</button>
					</div>
				</div>
				<div class="col-lg-6">
